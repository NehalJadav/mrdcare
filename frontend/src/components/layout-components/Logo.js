...
export const Logo = ({ mobileLogo, logoType }) => {
	...
	const getLogo = () => {
		if (logoType === 'light') {
			if (navCollapsed) {
				return window.env.REACT_APP_LOGO_SMALL_WHITE
			}
			return window.env.REACT_APP_LOGO_WHITE
		}

		if (navCollapsed) {
			return window.env.REACT_APP_LOGO_SMALL
		}
		return window.env.REACT_APP_LOGO
	}
